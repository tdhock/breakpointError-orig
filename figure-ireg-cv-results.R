## Do cross-validation to compare interval regression with the usual
## cghseg.k constant model.
...
library(ggplot2)
sub.stats <-
  subset(cv.stats,model%in%c("cghseg.k","hinge.1e5","square"))
...
p <- ggplot(sub.stats,aes(train.size,mean))+
...
pdf("figure-ireg-cv-results.pdf")
print(dl)
dev.off()
