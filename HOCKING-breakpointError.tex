...
\begin{document}
 ...
 \arxiv{math.PR/00000000} % Si disponible
  ...
  \begin{abstract}
    ...
    change-point locations, given the known locations of true
    change-points. We discuss an application of the breakpointError to
    calibrating penalty functions for change-point detection. Finally,
    ...
    breakpointError is available as an R package on R-Forge.
  \end{abstract}
...
\begin{figure}
  ...
  \caption{Two signals with the same breakpoints (vertical dashed
    ...
    100 sampled data points drawn from normal distributions defined on
    bases $D\in\{1, \dots, 500\}$. Horizontal line segments and shaded
    bands show mean $\pm$ one standard deviation. The goal of
    segmentation is to recover the distributions and/or change-points,
...
\begin{figure}
  ...
  \caption{Comparing true and estimated distributions. \textbf{Top 2
      panels}: the same reference/true signals as in
    Figure~{fig:motivation}. \textbf{Others}: estimated maximum
    likelihood models for $k\in\{2, \dots, 5\}$ segments.}
...
\begin{figure}
  ...
  \caption{Same as Figure~{fig:motivation-modelOnly}, but showing
...
time-varying normal distributions with the same change-points: the
