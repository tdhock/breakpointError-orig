...
We are given a sample of size $d \leq P$ of data $(p_1, y_1), \dots,
...
In this section, we assume that the true breakpoints $B\subseteq
\mathbb B$ are known, and we would like to quantify the accuracy of a
breakpoint guess $G\subseteq B$. In that case, we can compute the
breakpointError $E^B_{\text{exact}}(G)$.
...
4 properties. To compute the breakpointError, the true breakpoints $B$
must be known, as in the evaluation framework proposed by
\citet{perf-eval-framework}. However, the true breakpoints $B$ are not
often known in real data sets. So later in
...
latent signal $\mu\in\RR^P$ over $P=70000$ base pairs, with
breakpoints every 10000 base pairs, shown as the blue line in
...
$E_{01}^{\hat R,A}\left[\phi(\mathbf{\hat y}^k_i,p_i)\right]$. But in
...
\begin{equation}
  ...
  k_i(\lambda) = \argmin_k \lambda k \sqrt{d_i}+||\mathbf y_i-\mathbf{\hat y}_i^k||^2_2
\end{equation}
...
\item \textbf{(a)} estimated models are the same up to a scaling
  factor: $\mathbf{\hat y}_2^k = \mathbf{\hat y}_1^k\sigma$ for all model sizes $k$.
...
$p_i\in\mathcal P^{d_i}$. To find an optimal penalty for these data,
...
can be used in real data sets (Table~\ref{tab:ann-err-funs}). 

\begin{table}[H]
  \centering
  \begin{tabular}{ccc}
    Error function & Need & counts incorrect \\
    \hline
    breakpointError & true breaks & guesses \\
    complete annotation error & all 1 regions & guesses\\
    incomplete annotation error & some 0/1 regions & guesses\\
    01 annotation error & some 0/1 regions & guesses
  \end{tabular}
...
\subsection{Link with breakpoint error using 
