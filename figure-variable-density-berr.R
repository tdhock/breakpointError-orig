...
##pdf("figure-variable-density-berr.pdf",w=7,h=2)
tikz("figure-variable-density-berr.tex",w=6,h=2)
print(p)
dev.off()
...
kplot <- ggplot(err.df,aes(segments,cost))+
  ...
  scale_x_continuous(paste("Number of segments $k$ in",
                           "estimated maximum likelihood model"),
                     breaks=c(1,7,20),minor_breaks=NULL)+
  scale_y_continuous("breakpointError $\\textrm{BErr}_i(k)$",
                     breaks=c(0,1,6,13,14),minor_breaks=NULL)

##pdf("figure-variable-density-berr-k.pdf",h=3)
tikz("figure-variable-density-berr-k.tex",h=2.5,w=6)
print(kplot)
dev.off()
