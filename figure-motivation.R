...
is.sampled <- 1:LAST.BASE %in% sample(LAST.BASE, 100) # sampled points.
...
for(change.param in names(params)){
  ...
  for(segment.i in 1:nrow(these.segments)){
    ...
    full.segment <- data.frame(base, signal, segment.i, change.param,
                               what="data", facet)
    full.signal <- rbind(full.signal, full.segment)
  }
  sampled.signal <- full.signal[is.sampled, ]
  signals <- rbind(signals, sampled.signal)
  max.segments <- 5
...
names(model.colors) <- c("truth", "estimate")
mtype <- function(x){
  factor(ifelse(x=="truth", "truth", "estimate"), c("truth", "estimate"))
}
...
model.breaks <- c("truth", "estimate")
modelOnly <- 
ggplot()+
  geom_point(aes(base, signal), data=signals, pch=1)+
  geom_rect(aes(xmin=first.base-1/2, xmax=last.base+1/2,
                ymin=seg.mean-seg.sd, ymax=seg.mean+seg.sd,
                fill=mtype(model.type)),
  ...
  scale_fill_manual("distribution", values=model.colors, breaks=model.breaks)+
  scale_color_manual("distribution", values=model.colors, breaks=model.breaks)
w <- 7
h <- 5
...
ggplot()+
  ...
  scale_y_continuous(breaks=seq(-4, 4, by=2))+
  scale_fill_manual("breakpoints", values=model.colors, breaks=model.breaks)+
  scale_color_manual("breakpoints", values=model.colors, breaks=model.breaks)
